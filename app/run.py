...
def create_application():
    ...
    model_path = os.path.join(ROOT, "outputs", "models", "contest_100_v4.pt", )
    ...
    service = SignDetectionService(hand_detection=hand_detection, pose_detection=pose_detection, fusion=fusion,
        model=model, pretrained_model=pretrained_model, idx2gloss=idx2gloss, )

    camera = cv.VideoCapture(0, cv.CAP_DSHOW)
    application = SignDetectionApp(flask_app=flask_app, service=service, camera=camera, gloss_list=gloss_list,
