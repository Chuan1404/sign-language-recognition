...
for entry in tqdm(label_entries, total=len(label_entries)):
    ...
    gloss = entry["gloss"]
    video_path = os.path.join(WLASL_RAW_DATA, f"{video_id}.mp4")

    dir_name = os.path.join(SAVE_DIR, video_id)
    ...
    while True:
        ...
        lips_features.append(lip_coords.flatten())
    ...
    lips_features = np.array(lips_features, dtype=np.float32)
    ...
    np.save(os.path.join(dir_name, "lips.npy"), lips_features)
