...
class ISLR_Transformer_Selector(nn.Module):
    ...
    def forward(self, features, labels=None, video_mask=None):
        ...
        # Convert to temporal distribution
        ...
        return {"logits": logits, "loss": loss, "frame_importance": frame_importance,  # (B, T)
                "encoded": x,  # (B, T, d_model)
                }

import torchvision.models as models

class MultimodalFusionModel(nn.Module):
    def __init__(self, num_classes=1000, d_model=256, dropout=0.2):
        super().__init__()
        
        # 1. Nhánh Transformer cho Landmarks
        self.transformer_branch = ISLR_Transformer_Selector(
            hidden_dim=d_model, 
            num_classes=num_classes, 
            dropout=dropout
        )
        
        # 2. Nhánh CNN cho 5 Images (ResNet18)
        resnet = models.resnet18(weights=models.ResNet18_Weights.DEFAULT)
        # Bỏ đi lớp fully connected cuối của ResNet
        self.cnn_backbone = nn.Sequential(*list(resnet.children())[:-1]) # Output shape: (B, 512, 1, 1)
        
        # TCN (hoặc dùng Transformer/LSTM) trên 5 frames
        self.cnn_tcn = ShortTermTCN(channels=512, kernel_size=3, dropout=dropout)
        ...
        # 3. Lớp Fusion
        self.cross_attn = CrossAttentionLayer(d_model, nhead=8, dropout=dropout)
        self.classifier = nn.Linear(d_model, num_classes)
        
    def forward(self, landmarks, images, labels=None, video_mask=None):
        # landmarks: (B, T, D)
        # images: (B, 5, C, H, W)
        B, K, C, H, W = images.shape
        
        # --- Nhánh 1: Transformer trên Landmarks ---
        # Output trả về dict có chứa "encoded" (B, T, d_model) và "frame_importance"
        transformer_out = self.transformer_branch(landmarks, video_mask=video_mask)
        x_transformer = transformer_out["encoded"]
        
        # --- Nhánh 2: CNN trên Images ---
        # Gộp Batch và Time (5 frames) để đưa qua ResNet 2D
        images_flat = images.view(B * K, C, H, W)
        cnn_out = self.cnn_backbone(images_flat) # (B*5, 512, 1, 1)
        cnn_out = cnn_out.view(B, K, 512)
        
        # Dùng 1D CNN học sự thay đổi giữa 5 frame ảnh
        cnn_tcn_out = self.cnn_tcn(cnn_out) # (B, 5, 512)
        cnn_pooled = cnn_tcn_out.mean(dim=1) # (B, 512)
        cnn_pooled = self.cnn_proj(cnn_pooled) # (B, d_model)
        
        # --- Cross-Attention Fusion ---
        # Query: Images (CNN), Key/Value: Landmarks (Transformer)
        query = cnn_pooled.unsqueeze(1) # (B, 1, d_model)
        
        kv_pad_mask = None
        if video_mask is not None:
            kv_pad_mask = ~video_mask.bool() # True = ignore padding
            
        fused = self.cross_attn(q=query, kv=x_transformer, kv_pad_mask=kv_pad_mask) # (B, 1, d_model)
